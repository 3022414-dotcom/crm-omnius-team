...
const STAGES = [
  ...
  { value: 'discovery', label: 'Обследование' },
...
const emptyDefaults = {
  ...
  account_id: '', location: '', deal_type: '', source: '',
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-background border border-border rounded-lg shadow-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-base font-semibold">{initial ? 'Редактировать сделку' : 'Новая сделка'}</Dialog.Title>
          ...
          <form onSubmit={handleSubmit(onSave)} className="space-y-4">
            ...
            <Field label="Стадия *" error={errors.stage}>
            ...
            {stage === 'lost' && (
              <Field label="Причина проигрыша *" error={errors.lost_reason}>
                <textarea {...register('lost_reason')} rows={2} className={inputClass} placeholder="Укажите причину..." />
              </Field>
            )}

            <Field label="Аккаунт" error={errors.account_id}>
            ...
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide pt-1">Параметры сделки</p>

            <Field label="Тип сделки" error={errors.deal_type}>
            ...
            <Field label="Домен проекта" error={errors.project_domain}>
            ...
            <Field label="Наши услуги" error={errors.our_services}>
            ...
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide pt-1">Финансы и сроки</p>

            <div className="grid grid-cols-2 gap-3">
              <Field label="Сумма" error={errors.value}>
            ...
            <Field label="Дата начала" error={errors.expected_start_date}>
              <input {...register('expected_start_date')} type="date" className={inputClass} />
            </Field>

            <Field label="Дата закрытия" error={errors.close_date}>
              <input {...register('close_date')} type="date" className={inputClass} />
            </Field>

            <Field label="Deal Storage (URL)" error={errors.deal_storage}>
              <input {...register('deal_storage')} type="url" className={inputClass} placeholder="https://drive.google.com/..." />
            </Field>

            <div className="flex justify-end gap-3 pt-2">
              <button type="button" className="px-4 py-2 text-sm rounded-md border border-border hover:bg-muted transition-colors" onClick={onClose}>Отмена</button>
