export default function Login() {
  ...
  const errorMessages = {
    ...
    email_not_verified: 'Google-аккаунт не подтверждён.',
    oauth_cancelled: 'Вход отменён. Попробуйте снова.',
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="w-full max-w-sm space-y-6 p-8 rounded-xl border border-border bg-card shadow-sm">
        <div className="text-center space-y-1">
          <h1 className="text-2xl font-bold tracking-tight">omnius.team</h1>
          <p className="text-sm text-muted-foreground">CRM</p>
        </div>

        {error && (
          <div className="rounded-md bg-destructive/10 border border-destructive/30 px-4 py-3 text-sm text-destructive">
            {errorMessages[error] || 'Произошла ошибка. Попробуйте снова.'}
          </div>
        )}
